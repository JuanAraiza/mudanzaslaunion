...
if(isset($_POST['enviarcerrados'])){
    ...
    $query2 ="update servicio set cerrado='SI', estatus='6' where id=".$_POST['idser'];
    $resultr2 = $link->query($query2);
...
if(isset($_GET['regre'])){
    ...
    $query2 = "update servicio set cerrado='NO',estatus=1 where clave='".$_GET['c']."'";
    $resultr2 = $link->query($query2);
    $rowr2 = mysqli_fetch_row($resultr2);
//echo "update servicio set cerrado='SI' where id=".$_POST['idser'];
//  echo $rowr[0].'  -  '.$rrr.'  -  '.$_POST[$rrr].'<br>'; 
...
<div class="row">
    <div class="col-md-12">
<a class="btn bg-green" href="excelservicios.php" target="_blank">Exportar Finiquitados tabla a Excel</a>
<br>
<form method="post" action="tareas.php?ser=3">
...
$cade="  and estatus=5 and cerrado!='SI'";
//$cade=" and estatus=2";
...
$query = "SELECT * from servicio where id>=1 ".$cade." order by id desc";
	...
	$query = "SELECT * from servicio where estatus='5' and cerrado!='SI' order by id desc";
}
$_SESSION['query']=$query;
$_SESSION['servicios']='FINIQUITADOS';
		//echo $query.'<br>';
    $r=1;
    $cacheck='';
    $rc=0;
      ...
      $c='#d0d22f';
        ...
        <form method="post" action="tareas.php?ser=3">
         <div class="form-group" style="align-content: center;">
               <input type="hidden"  name="idser" value="<?php echo $row[0]; ?>">
                <button type="submit" class="btn btn-primary"  name="enviarcerrados" id="enviarcerrados">Cerrar<br>Servicio</button>
            </div>
...
-->
    ...
    <td style="vertical-align:middle;"><a class="btn" style="background-color: #d0d22f; color: #fff; font-size: 11px;" href="tareas.php?ser=3&c=<? echo $row[9]; ?>&regre=1" >Regresar a Servicios</a></td>
     <td style="vertical-align:middle;"><? if($row[47]=='NO' || $row[47]=='AC'){?><a class="btn bg-black" style="font-size: 11px;"  href="tareas.php?ser=1&c=<? echo $row[9]; ?>&cerr=1" >Cerrar</a><? }else{ ?><a class="btn bg-black" style="font-size: 11px;"  href="tareas.php?ser=1&c=<? echo $row[9]; ?>&abi=1" >Reabrir</a><? }?></td>
